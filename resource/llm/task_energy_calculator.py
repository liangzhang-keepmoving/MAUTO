...
class TaskEnergyCalculator:
    ...
    def calculate_task_processing_energy(self, actions, user_states, uav_states,user_assignments,max_uav_computation_delays):
        ...
        for user_id in range(num_users):
            ...
            # 1.3 用户总能耗 = 本地 + 卸载
            user_total_energy[user_id] = user_local_energy[user_id]
            # print(f"user_total_energy: {user_total_energy[user_id]}")
            

        # 计算每个UAV负责的用户能耗累加（基于新的用户视角数据）
        uav_task_total_energy = {uav_id: 0.0 for uav_id in range(num_uavs)}
        uav_transmission_total_energy = {uav_id: 0.0 for uav_id in range(num_uavs)}
        
        for user_id, uav_id in user_assignments.items():
            # 用户实际总能耗（直接从task_energy_breakdown获取）
            user_actual_energy = user_total_energy.get(user_id, 0.0)
            uav_task_total_energy[uav_id] += user_actual_energy
            uav_transmission_total_energy[uav_id] += user_offload_energy.get(user_id, 0.0)
        for uav_id in range(num_uavs):
            uav_task_total_energy[uav_id] += self.uav_cpu_power * max_uav_computation_delays[uav_id]
        ...
        return {
            ...
            'uav_task_total_energy': uav_task_total_energy ,
            'uav_transmission_total_energy': uav_transmission_total_energy 
        }
