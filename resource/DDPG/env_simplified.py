...
class SimplifiedMultiUAVEnvironment:
    ...
    def __init__(self, num_uavs=2, num_users=5,trajectory_file=None):
        ...
        # ==================== 无人机参数 ====================
        self.uav_cpu_frequency = 0.8       # UAV CPU频率 GHz
        self.cpu_cycles_per_mb = 1000      # 每MB任务的CPU周期数 (Megacycles)
        
        # 移动参数
        self.uav_max_speed = 15          # 最大飞行速度 m/s
        ...
        # 任务参数
        self.min_task_size = 0.5           # 最小任务大小 (MB)
        self.max_task_size = 1         # 最大任务大小 (MB)
        ...
        self.min_flight_delay = 0
        self.max_flight_delay = 2


        self.min_computation_delay = 0
        self.max_computation_delay = 8.37

        self.min_task_energy = 0
        self.max_task_energy = 3.5

        self.min_movement_energy = 1737
        self.max_movement_energy = 2171.2

        self.min_transmission_energy = 0
        self.max_transmission_energy = 0.6

        self.min_transmission_delay = 0
        self.max_transmission_delay = 2.87

        self.min_task_transmission_energy = 0
        self.max_task_transmission_energy = self.max_task_energy+self.max_transmission_energy
        # 性能统计
        self.stats = {
    ...
    def _initialize_uavs(self):
        ...
        for i in range(self.num_uavs):
            self.uav_states[i] = [center_x, center_y]
    ...
    def step(self, actions=None):
        ...
        # 3. 计算任务时延（新增）
        delay_results = self.delay_calculator.calculate_task_delays(actions, self.user_states, self.uav_states,user_assignments)
        uav_delays = delay_results['actual_delays']
        max_uav_computation_delays = delay_results['max_uav_computation_delays']
        max_transmission_delays = delay_results['max_transmission_delays']
        
        # 4. 计算任务处理能耗（核心，包含最大值）
        uav_task_total_energy = self.task_energy_calculator.calculate_task_processing_energy(actions, self.user_states, self.uav_states,user_assignments,max_uav_computation_delays)['uav_task_total_energy']
        uav_transmission_total_energy = self.task_energy_calculator.calculate_task_processing_energy(actions, self.user_states, self.uav_states,user_assignments,max_uav_computation_delays)['uav_transmission_total_energy']
        ...
        for uav_id in range(self.num_uavs):
            ...
            normalized_movement_energy = (movement_energy_costs.get(uav_key, 0.0)-self.min_movement_energy) / (self.max_movement_energy - self.min_movement_energy)
            normalized_movement_delay = (uav_movement_actual_delays.get(uav_key, 0.0)-self.min_flight_delay) / (self.max_flight_delay - self.min_flight_delay)
            normalized_transmission_energy = (uav_transmission_total_energy[uav_id]-self.min_transmission_energy) / (self.max_transmission_energy*5 - self.min_transmission_energy)
            normalized_transmission_delay = (max_transmission_delays[uav_id]-self.min_transmission_delay) / (self.max_transmission_delay - self.min_transmission_delay)
            
          
            normalized_task_energy = (uav_task_total_energy[uav_id] - self.min_task_energy) / (self.max_task_energy - self.min_task_energy)
            
            task_transmission_energy = uav_transmission_total_energy[uav_id]+uav_task_total_energy[uav_id]
            normalized_task_transmission_energy = (task_transmission_energy-self.min_task_transmission_energy) / (self.max_task_transmission_energy - self.min_task_transmission_energy)
        
            uav_delay_raw = uav_delays.get(uav_id, 0.0)
            normalized_delay = (uav_delay_raw-self.min_computation_delay) / (self.max_computation_delay - self.min_computation_delay)

           
            # 计算加权后的惩罚（两个组件：系统总能耗 + 时延）
           
            total_rewards+=(normalized_transmission_energy+normalized_task_energy+normalized_delay)
            # 保存原始值（包含所有基准值和归一化信息）
            raw_metrics[uav_key] = {
                # 原始能耗值（用户视角累加）
                'actual_task_energy_raw': uav_task_total_energy[uav_id],     
                'actual_transmission_energy_raw': uav_transmission_total_energy[uav_id],     
                'uav_movement_energy_raw': movement_energy_costs.get(uav_key, 0.0) ,      # UAV飞行能耗（独立）
               
                'delay_raw': uav_delay_raw,
                'movement_delay_raw': uav_movement_actual_delays.get(uav_key, 0.0),
                'transmission_delay_raw': max_transmission_delays.get(uav_id, 0.0),
            }
