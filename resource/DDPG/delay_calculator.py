...
class DelayCalculator:
    ...
    def calculate_task_delays(self, actions, user_states, uav_states, user_assignments):
        ...
        for uav_id, served_users in uav_user_groups.items():
            ...
            for user_id in served_users:
                ...
                if offload_task_size > 0:
                    offload_delay, uav_comp_delay, transmission_delay = self._calculate_offloading_delay(
            ...
            max_transmission_delays[uav_id] = max(user_transmission_delays.values()) if user_transmission_delays else 0.0
        ...
        for uav_id in range(num_uavs):
            ...
            if uav_id not in max_uav_computation_delays:
                max_uav_computation_delays[uav_id] = 0.0
            if uav_id not in max_transmission_delays:
                max_transmission_delays[uav_id] = 0.0
        
        # 返回包含四种时延的字典
        return {
            'actual_delays': actual_delays,
            'max_uav_computation_delays': max_uav_computation_delays,  # 新增：每个UAV的最大计算时延
            'max_transmission_delays': max_transmission_delays
        }
    ...
    def _calculate_offloading_delay(self, task_size, user_id, uav_id, uav_cpu_per_user, 
        ...
        return total_offload_delay, uav_computation_delay, transmission_delay
