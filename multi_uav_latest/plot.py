...
# 3. 绘制 Movement Energy
ax3 = axes[1, 0]
ax3.plot(df['episode'], df['movement_energy'], linewidth=2, color='#F18F01', marker='^', markersize=3)
ax3.set_xlabel('Episode', fontsize=12)
ax3.set_ylabel('Movement Energy', fontsize=12)
ax3.set_title('Movement Energy', fontsize=13, fontweight='bold')
ax3.grid(True, alpha=0.3)

# 4. 绘制 Max Delay
ax4 = axes[1, 1]
ax4.plot(df['episode'], df['max_delay'], linewidth=2, color='#C73E1D', marker='d', markersize=3)
ax4.set_xlabel('Episode', fontsize=12)
ax4.set_ylabel('Max Delay', fontsize=12)
ax4.set_title('Max Delay', fontsize=13, fontweight='bold')
ax4.grid(True, alpha=0.3)

# 5. 绘制各UAV的时延对比（占据两个子图位置）
...
# 保存图片到pic文件夹
output_file = 'pic/training_results.png'
...
print(f"Max Delay - Min: {df['max_delay'].min():.6f}, Max: {df['max_delay'].max():.6f}, Mean: {df['max_delay'].mean():.6f}")
