...
class UAVMovementManager:
    ...
    def __init__(self, num_uavs, area_length, area_width, uav_max_speed, time_step, 
                 move_power, hover_power, uav_battery_capacity):
        """
        初始化UAV移动管理器
    ...
    def process_uav_movements(self, actions, uav_states):
        ...
        for uav_id in range(self.num_uavs):
            ...
            if uav_key in actions:
                # 获取移动参数
                if 'movement_direction' in actions[uav_key] and 'movement_distance' in actions[uav_key]:
                    direction = actions[uav_key]['movement_direction']  # 弧度
                    distance = actions[uav_key]['movement_distance']    # 米
        ...
        return movement_penalties, movement_energy_costs
    ...
    def _calculate_energy_consumption(self, actual_distance):
        ...
        if actual_distance > 0:
            ...
            # 剩余时间为悬停
            hover_time = self.time_step - flight_time
            
            # 能耗 = 飞行时间×移动功耗 + 悬停时间×悬停功耗
            energy_consumed = (self.move_power * flight_time) + (self.hover_power * hover_time)
        else:
            # 整个时间步都在悬停
            energy_consumed = self.hover_power * self.time_step
