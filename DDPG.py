...
class Actor(nn.Module):
    ...
    def __init__(self, n_uavs, n_users, uav_feature_dim=2, user_feature_dim=3):
        ...
        self.motion_head = nn.Sequential(
            nn.Linear(256, 128),
            nn.ReLU(),
            nn.Linear(128, n_uavs * 2)
        )
        
    def forward(self, uav_pos, user_pos, user_tasks, hard=False):
        """
        Args:
            uav_pos: [batch, N, 2] - 无人机2D位置
            user_pos: [batch, M, 2] - 用户2D位置
            user_tasks: [batch, M, 1] - 用户任务大小
            hard: 是否返回硬分配 (0/1)
        
        Returns:
            allocation: [batch, N, M] - 用户分配矩阵
            offloading: [batch, N, M] - 卸载比例矩阵
            motion: [batch, N, 2] - 运动控制 [距离, 角度]
        """
        ...
        # === 输出2: 卸载比例 ===
        offloading_raw = self.offloading_head(global_features)  # [batch, N*M]
        offloading_raw = offloading_raw.view(batch_size, self.n_uavs, self.n_users)
        offloading_ratio = torch.sigmoid(offloading_raw)  # [batch, N, M], [0, 1]
        
        # 结合分配决策得到最终卸载比例
        offloading_final = allocation
        
        # === 输出3: 运动控制 ===
        motion_raw = self.motion_head(global_features)  # [batch, N*2]
        motion_raw = motion_raw.view(batch_size, self.n_uavs, 2)
        
        # 归一化运动参数
        distance = torch.sigmoid(motion_raw[:, :, 0:1])  # [0, 1]
        angle = torch.tanh(motion_raw[:, :, 1:2])  # [-1, 1]
        motion = torch.cat([distance, angle], dim=-1)  # [batch, N, 2]
...
class Critic(nn.Module):
    ...
    def forward(self, state, allocation, offloading, motion):
        ...
        offloading_flat = offloading.view(offloading.shape[0], -1)
        offloading_feat = self.offloading_encoder(offloading_flat)
...
class ReplayBuffer:
    ...
    def _collate_states(self, states):
        ...
        uav_pos = torch.stack([s['uav_pos'] for s in states])
        user_pos = torch.stack([s['user_pos'] for s in states])
        user_tasks = torch.stack([s['user_tasks'] for s in states])
    ...
    def select_action(self, state, add_noise=True, hard=False):
        ...
        with torch.no_grad():
            ...
            if add_noise:
                ...
                # ===== 修复：分别处理距离和角度 =====
                # 距离：clamp到[0, 1]
                distance = motion[:, :, 0:1] + noise[:, :, 0:1] * 0.1 * self.noise_scale
                distance = torch.clamp(distance, 0, 1)
                
                # 角度：clamp到[-1, 1]
                angle = motion[:, :, 1:2] + noise[:, :, 1:2] * 0.1 * self.noise_scale
                angle = torch.clamp(angle, -1, 1)  # 修复：应该是[-1, 1]
                
                # 重新组合
                motion = torch.cat([distance, angle], dim=-1)
...
# env = SimplifiedMultiUAVEnvironment(num_uavs=3, num_users=5,trajectory_file='user_trajectories.json')
...
# # print(info['user_assignments'])
# # print(info['uav_delays'])
